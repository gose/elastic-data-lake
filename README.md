...
The other common scenario here is Windows Event Logs.  [Winlogbeat](https://www.elastic.co/beats/winlogbeat) specliazes in the collection of these and will store events by default in an index named `winlogbeat-<version>-yyyy.MM.dd`.  For large Windows environments producing > 50 GB / day of Windows Event Logs, it's encouraged to follow the defaults Winlogbeat provides.  [Elastic ILM](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-lifecycle-management.html) will roll these indices as necessary to keep the shard sizes of these indices below the recommended 50 GB.

In general, even for data sources that produce a few MB per day, we're generating a couple shards per day (1 primary, 1 replica).  There is a case where "over sharding" (i.e., lots of small shards) can negatively impact the performance of an Elastic cluster.  We'll keep an eye out for this scenario but we shouldn't prematurely optimize for it.  The first goal, is creating a system that is easy to manage and rationalize about.
